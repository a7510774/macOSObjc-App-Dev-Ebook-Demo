...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  ...
  [NSFileHandle aspect_hookSelector:@selector(seekToFileOffset:)
                        withOptions:0
                         usingBlock:^(id info, unsigned long long sender) {

                           NSLog(@"Button was pressed by: %llu", sender);

                         }
                              error:NULL];

  [NSFileHandle aspect_hookSelector:@selector(fileHandleForWritingAtPath:)
                        withOptions:0
                         usingBlock:^(id info, NSString *path) {
  ...
  [NSFileHandle aspect_hookSelector:@selector(writeData:)
                        withOptions:0
                         usingBlock:^(id info, NSData *sender) {
...
- (IBAction)startDown:(id)sender {
  ...
  NSURL *url = [NSURL URLWithString:@"http://devstreaming.apple.com/videos/"
                                    @"wwdc/2015/105ncyldc6ofunvsgtan/105/"
                                    @"105_hd_introducing_watchkit_for_watchos_"
                                    @"2.mp4?dl=1"];

  url = [NSURL URLWithString:@"http://devstreaming.apple.com/videos/wwdc/2015/"
                             @"1026npwuy2crj2xyuq11/102/"
                             @"102_platforms_state_of_the_union.pdf?dl=1"];
...
- (void)urlSessionNoDelegateTest {
  ...
  NSURL *url =
      [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", kServerBaseUrl,
                                                      @"/VersionCheck"]];
  ...
  [session
      getTasksWithCompletionHandler:^(NSArray *dataTasks, NSArray *uploadTasks,
        ...
        for (NSURLSessionDataTask *task in dataTasks) {
          NSLog(@"taskDescription %@  %ld", task.taskDescription,
                task.taskIdentifier);
        }
...
- (void)URLSession:(NSURLSession *)session
  ...
  NSLog(@"data =%@ taskIdentifier=%ld error %@", responseStr,
        task.taskIdentifier, error);
...
- (void)URLSession:(NSURLSession *)session
  ...
  //获取原始的文件名
  NSString *fileName =
      [[downloadTask.originalRequest.URL absoluteString] lastPathComponent];
...
- (void)URLSession:(NSURLSession *)session
  ...
  NSLog(@"Session %@ download task %@ wrote an additional %lld bytes (total "
        @"%lld bytes) out of an expected %lld bytes.\n",
...
- (void)urlSessionUploadTest {
  ...
  request.HTTPMethod = @"POST";
  //[request addValue:@"multipart/form-data"
  //forHTTPHeaderField:@"Content-Type"];
  ...
  NSString *downloadDir =
      [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]
          stringByAppendingPathComponent:@"openMacIcon.png"];
...
- (IBAction)sendAudio {
  ...
  NSString *downloadDir =
      [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]
          stringByAppendingPathComponent:@"AppBkIcon.png"];

  //要上传的路径
  NSURL *fileURL = [NSURL fileURLWithPath:downloadDir];

  uint64_t bytesTotalForThisFile =
      [[[NSFileManager defaultManager] attributesOfItemAtPath:fileURL.path
                                                        error:NULL] fileSize];

  [request setValue:[NSString stringWithFormat:@"%llu", bytesTotalForThisFile]
...
- (void)URLSession:(NSURLSession *)session
  ...
  NSLog(@"Session %@ upload task %@ wrote an additional %lld bytes (total %lld "
...
- (void)formUpload2 {
  ...
  [request setValue:[NSString
                        stringWithFormat:@"multipart/form-data; boundary=%@",
                                         boundary]
      forHTTPHeaderField:@"Content-Type"];
  //[request setValue:[@"authtok=" stringByAppendingString:[broker
  //getNasAuthtok]] forHTTPHeaderField:@"Cookie"];
  ...
  NSURLSessionUploadTask *sessionUploadTask =
      [session uploadTaskWithRequest:request fromFile:uploadfileURL];
...
- (NSData *)createBodyWithBoundary:(NSString *)boundary
  ...
  for (NSString *path in paths) {
    ...
    [httpBody
        appendData:[[NSString
                       stringWithFormat:@"http://127.0.0.1/upload_file.php"]
                       dataUsingEncoding:NSUTF8StringEncoding]];
  }
...
- (IBAction)upload:(id)sender {
  ...
  NSURLSessionUploadTask *sessionUploadTask =
      [session uploadTaskWithRequest:request fromData:dataSend];
  [sessionUploadTask resume];
}
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection
  ...
  NSDictionary *newUserInfo =
      [NSDictionary dictionaryWithObject:[NSDate date] forKey:@"Cached Date"];
...
- (void)urlConnectionTest {
  ...
  NSURLRequest *request = [NSURLRequest
       requestWithURL:
           [NSURL
               URLWithString:@"http://127.0.0.1/iosxhelper/SAPI/VersionCheck"]
          cachePolicy:NSURLRequestUseProtocolCachePolicy
      timeoutInterval:60.0];
  ...
  //    NSString *synResponse = [[NSString alloc] initWithData:data
  //    encoding:NSUTF8StringEncoding];
  ...
  //    new]  completionHandler:^(NSURLResponse *response, NSData *data, NSError
  //    *connectionError) {
  //
  //        if (connectionError) {
  //            NSLog(@"request failed: %@", error);
  ...
  //        }
  //
  //        NSString *asynResponse = [[NSString alloc] initWithData:data
  //        encoding:NSUTF8StringEncoding];
...
- (void)startURLDownloadTest {
  ...
  NSURL *url =
      [NSURL URLWithString:@"http://devstreaming.apple.com/videos/wwdc/2015/"
                           @"105ncyldc6ofunvsgtan/105/"
  ...
  NSString *filePath =
      [[NSHomeDirectory() stringByAppendingPathComponent:@"Downloads"]
          stringByAppendingPathComponent:
              @"105_introducing_watchkit_for_watchos_2.pdf"];
...
- (void)download:(NSURLDownload *)download
  ...
  NSLog(@"download didReceiveDataOfLength %ld currentLength %ld ", length,
        self.currentLength);
}
